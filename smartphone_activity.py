...
activity_list_abr = ['Walk', 'Walk up', 'Walk down',
                      'Sit', 'Stand', 'Lay']
...
scores_rouned = map(lambda x: round(x, 5), [score_test, precision_test, recall_test, f1_test])
print('\nRandom Forest scores on test data (global):\n\tAccuracy: {0}\n\tPrecision: {1}\n\tRecall: {2}\n\tF1: {3}'.format(
         scores_rouned[0], scores_rouned[1], scores_rouned[2], scores_rouned[3]))
...
#we could use xn for column names but because we don't take anything out of X_train so it doesn't make a difference
...
prediction = clf_bb.predict(X_test_bb)
score_test = clf_bb.score(X_test_bb, y_test.activity)
precision_test = skmet.precision_score(y_test.activity, prediction, average='micro')
recall_test = skmet.recall_score(y_test.activity, prediction, average='micro')
f1_test = skmet.f1_score(y_test.activity, prediction, average='micro')
scores_rouned = map(lambda x: round(x, 5), [score_test, precision_test, recall_test, f1_test])
